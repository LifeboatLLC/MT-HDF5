...
H5VL_bypass_init(hid_t vipl_id)
{
    ...
    for (i = 0; i < nthreads_tpool; i++) {
        info_for_thread[i].thread_id =
            i; /* Remove info_for_thread and pass in the thread_id directly to pthread_create */
...
H5VL_bypass_term(void)
{
    ...
    /* If H5Dread isn't even called in the application, the thread pool is waiting for this condition
     * variable. This broadcast tells the thread pool to stop waiting.  Turning on the 'thread_task_finished'
     * variable stops the while loop for pthread_cond_wait in thread pool. */
    ...
    // printf("%s: %d, dset_count = %d, dset_stuff[0].space_id = %lld\n", __func__, __LINE__, dset_count,
    // dset_stuff[0].space_id);
...
get_linkinfo_helper(void *obj, const char *name, void **req)
{
    ...
    // H5VL_bypass_link_get(void *obj, const H5VL_loc_params_t *loc_params, H5VL_link_get_args_t *args, hid_t
    // dxpl_id, void **req);
...
dset_open_helper(void *obj, const char *name, H5VL_bypass_t *dset, hid_t dxpl_id, void **req)
{
    ...
    // printf("\n%s: %d, count=%d, dset_info_size=%d, layout=%d, location=%llu, name=%s, dtype_id = %llu,
    // H5T_STD_REF_DSETREG = %llu, H5T_NATIVE_INT = %llu, dcpl_id = %llu\n", __func__, __LINE__, dset_count,
    // dset_info_size, dset_stuff[dset_count].layout, addr, name, dset_stuff[dset_count].dtype_id,
    // H5T_STD_REF_DSETREG, H5T_NATIVE_INT, dcpl_id);
    ...
    // printf("\n%s: %d, count=%d, dset_info_size=%d, layout=%d, location=%llu, name=%s, dtype_id = %llu,
    // H5T_STD_REF_DSETREG = %llu, dcpl_id = %llu\n", __func__, __LINE__, dset_count, dset_info_size,
    // dset_stuff[dset_count].layout, addr, name, dset_stuff[dset_count].dtype_id, H5T_STD_REF_DSETREG,
    // dcpl_id); printf("\n%s: %d,  count=%d, dset_info_size=%d, layout=%d, location=%llu, dset_name=%s\n",
    // __func__, __LINE__, dset_count, dset_info_size, dset_stuff[dset_count].layout, addr,
    // dset_stuff[dset_count].dset_name);
 ...
 * H5S_ALL	valid ID	The file dataset's dataspace is used for the memory dataspace and the
 * selection specified with file_space_id specifies the selection within it. The combination of the file
 * dataset's dataspace and the selection from file_space_id is used for memory also. valid ID	H5S_ALL
 * mem_space_id specifies the memory dataspace and the selection within it. The selection within the file
 * dataset's dataspace is set to the "all" selection. H5S_ALL	H5S_ALL	        The file dataset's dataspace
 * is used for the memory dataspace and the selection within the memory dataspace is set to the "all"
 * selection. The selection within the file dataset's dataspace is set to the "all" selection.
 */
static herr_t
...
start_thread_for_pool(void *args)
{
    ...
    while (!thread_loop_finish || !stop_tpool) {
        ...
        // %llu, size = %lld, buf = %p, vec_arr_nused = %d\n", thread_id, local_count, thread_task_count,
        // info_pointer - 1, addrs_local[0], sizes_local[0], vec_bufs_local[0], md_for_thread.vec_arr_nused);
...
process_vectors(void *rbuf, sel_info_t *selection_info, Bypass_task_author_t *author)
{
    ...
    /* Loop until all elements are processed. The algorithm is copied from the function
     * H5FD__read_selection_translate in H5FDint.c, which is somewhat similar to H5D__select_io in H5Dselect.c
     */
    while (file_seq_i < file_nseq || nelmts > 0) {
        ...
        /* Save the info for the C log file */
        {
            ...
            // info_stuff[info_count].end_of_read);
        ...
        if (read_use_native) {
            ...
            for (i = 0; i < file_stuff_count; i++) {
                if (!strcmp(file_stuff[i].name, file_name)) {
                    /* Save this index in the list of FILE_T structures for quick lookup later */
                    selection_info.my_file_index = i;
                    break;
                }
            }
...
c_file_open_helper(const char *name)
{
    ...
    /* if (NULL == (file_stuff[file_stuff_count].vfd_file_handle = H5FDopen(name, H5F_ACC_RDONLY, H5P_DEFAULT,
    HADDR_UNDEF))) puts("failed to open VFD file");
...
H5VL_bypass_file_close(void *file, hid_t dxpl_id, void **req)
{
    ...
    /* Close the file opened with C.  Remove the file structure from the list when the reference count drops
     * to zero */
    for (i = 0; i < file_stuff_count; i++) {
