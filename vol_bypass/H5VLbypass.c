...
herr_t is_any_thread_active(bool *out, bool mutex_locked);
...
static herr_t bypass_queue_destroy(bool mutex_locked);
static herr_t bypass_queue_push(Bypass_task_t *task, bool mutex_locked);
/* Retrieve a task from the global queue. Task must be released by caller.*/
static Bypass_task_t * bypass_queue_pop(bool mutex_locked);
...
H5VL_bypass_init(hid_t vipl_id)
{
    ...
    md_for_thread.thread_is_active = malloc(nthreads_tpool * sizeof(bool));
...
H5VL_bypass_term(void)
{
    ...
    while (true) {
        if (is_any_thread_active(&any_thread_active, true) < 0) {
...
start_thread_for_pool(void *args)
{
    ...
    while (!thread_loop_finish || !stop_tpool) {
        ...
        for (i = 0; i < local_count; i++) {
            ...
            if ((tasks[i] = bypass_queue_pop(true)) == NULL) {
...
process_vectors(void *rbuf, sel_info_t *selection_info)
{
    ...
    while (file_seq_i < file_nseq || nelmts > 0) {
	   ...
	   mem_seq_i = 0;
        ...
        if (bypass_queue_push(task, true) < 0) {
...
herr_t
is_any_thread_active(bool *out, bool mutex_locked) {
    herr_t ret_value = 0;

    assert(out);

    if (!mutex_locked && pthread_mutex_lock(&mutex_local) != 0) {
    ...
    if (!mutex_locked && pthread_mutex_unlock(&mutex_local) != 0) {
...
static herr_t
bypass_queue_destroy(bool mutex_locked) {
    herr_t ret_value = 0;
    bool locked = false;
...
static herr_t
bypass_queue_push(Bypass_task_t *task, bool mutex_locked) {
    herr_t ret_value = 0;
    bool locked = false;
...
bypass_queue_pop(bool mutex_locked) {
