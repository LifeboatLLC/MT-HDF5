...
./h5_write -t 0 -d ${DIM1}x${DIM2} -c ${CHUNK_DIM1}x${CHUNK_DIM2} -q ${NDATA_SECTIONS}

# echo ""
# echo "Test 1b: Reading single dataset in a single file with straight HDF5 (no Bypass VOL)"
# ./h5_write -t ${NTHREADS_FOR_MULTI} -d ${DIM1}x${DIM2} -c ${CHUNK_DIM1}x${CHUNK_DIM2} -q ${NDATA_SECTIONS}
